// Default program bundled with the app so a fresh visitor (e.g. a client
// opening the shared link for the first time) sees a program immediately
// instead of an empty "Import" screen. Only used when no program has been
// saved to this browser yet (see boot logic in app.js) -- it never
// overwrites anything a visitor has already imported or logged.
export const SEED_SHEET_TEXT = `	Semperfit Training - Phase 1 - Friday (WORKOUT C)

	Incline BB Bench Press			Rep goal: 5-10			Rest time: 2-3 min.

	Perform 2-3 warm-up sets of 6 reps using the easier exercise progressions.

	Week	WU set	Set 1	Set 2	set 3		Notes

1	WEEK 1. 1.5					4615
2	WEEK 2					5990
3	WEEK 3					6230
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8

	Slight Incline Alt DB chest Press			Rep goal: 5-10			Rest time: 2-3 min.

	Perform 2-3 warm-up sets of 6 reps using the easier exercise progressions.

	Week	Set 1	Set 2	Set 3			Notes

1	WEEK 1.    1.5
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8	0	0	0

	Dips			Rep goal : 4-6, 6-8, 8-10			Rest time: 2-3 min

	Week	Set 1	Set 2	Set 3			Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5 9.8
6	WEEK 6
7	WEEK 7
8	WEEK 8

	DB Skul Crushers			Rep goal: 6-8, 8-10, 10-12			Rest time: 2-3 min

	Week	Set 1	Set 2	Set 3			Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8	0	0	0

	Cable Rows			Rep goal: 12, 10, 8, 6			Rest time: 40 sec.

	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6 9.8
7	WEEK 7	0	0	0	0
8	WEEK 8	0	0	0	0

	Face Pulls			Rep goal: 10–15 reps + 4 sets x 4-6 reps			Rest time: Rest Pause 15 sec.

	Week	Set 1	Set 2	Set 3	Set 4	Set 5	Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7	0	0	0	0	0
8	WEEK 8	0	0	0	0	0

	tricep pull down
	Week	Set 1	set 2	Set 3	Set 4	Set 5	Set 6
					upright
	WEEK 1
	WEEK 2
	WEEK 3
	WEEK 4
	WEEK 5
	WEEK 6
	WEEK 7
	WEEK 8
	slight decline chest press
	Week	Set 1	set 2	Set 3	Set 4

	WEEK 1
	WEEK 2
	WEEK 3
	WEEK 4
	WEEK 5
	WEEK 6
	WEEK 7
	WEEK 8

	dips
	Week	Set 1	set 2	Set 3

	WEEK 1
	WEEK 2
	WEEK 3
	WEEK 4 10
	WEEK 5
	WEEK 6 10
	WEEK 7
	WEEK 8


	Semperfit Training - Phase 1 - Wednesday (WORKOUT B)

	RDLs			Rep goal: Follow program of 5x5 or 5x10			Rest time:  2-4 min

	Perform 2-3 warm-up sets of 6 reps using lighter weights

	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3 135/5
4	WEEK 4 185/5
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8


	up right Leg curls			Rep goal: Follow program of 5x5 or 5x10

	Perform 2-3 warm-up sets of 6 reps using lighter weights

	Week	Set 1	Set 2	Set 3	Set 4

	WEEK 1
	WEEK 2
	WEEK 3
	WEEK 4
	WEEK 5
	WEEK 6
	WEEK 7					Rest time:  90 sec
	WEEK 8

	SQUATS
	Perform 2-3 warm-up sets of 6 reps using lighter weights

	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3 6/6 135
4	WEEK 4 135/8
5	WEEK 5 135/7
6	WEEK 6
7	WEEK 7	0	0	0	0
8	WEEK 8	0	0	0	0

	Leg Extensions			Rep goal : 12, 10, 8, 6			Rest time:  90 sec

	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3.  SL
4	WEEK 4
5	WEEK 5
6	WEEK 6	0	0	0	0
7	WEEK 7	0	0	0	0
8	WEEK 8	0	0	0	0

	Leg Curls			Rep goal : 12, 10, 8, 6			Rest time:  90 sec

	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7	0	0	0	0
8	WEEK 8	0	0	0	0

	deadlift. con			Rep goal: 10-12			Rest time: 2 min

	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2					315/1
3	WEEK 3
4	WEEK 4
5	WEEK 5	0	0	0	0
6	WEEK 6	0	0	0	0
7	WEEK 7	0	0	0	0
8	WEEK 8	0	0	0	0

		set 1	set 2	set 3
	ab roll out /wk 1
	leg raises w ball











	Semperfit Training - Phase 1 - Monday (WORKOUT A)

	Weighted Pull-ups				Rep goal: 5,6,8		Rest time: 2-3 min.

	Perform 2 warm-up sets of 6 reps using the easier exercise progressions.

	Week	Set 1	Set 2	Set 3			Notes
			45/7
1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8

	seated military press				Rep goal: 4-6, 6-8, 8-10		Rest time: 2-3 min.


	Perform 2-3 warm-up sets of 6 reps using the easier exercise progressions.

	Week	Set 1	Set 2	Set 3			Notes

1	WEEK 1 95/5.
2	WEEK 2 DB
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8

	Incline Dumbbell Flyes			Rep goal: 10–15 reps + 4 sets x 4-6 reps			Rest time: Rest Pause (15s)


	Week	Set 1	Set 2	Set 3	Set 4	Set 5	Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8

	Hammer Curls			Rep goal: 12, 10, 8, 6			Rest time: 40 secs


	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3 8.10
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8

	Lateral Raises			Rep goal: 10–15 reps			Rest time: Rest Pause 15 sec.



	Week	Set 1	Set 2	Set 3	Set 4		Notes

1	WEEK 1
2	WEEK 2
3	WEEK 3
4	WEEK 4
5	WEEK 5
6	WEEK 6
7	WEEK 7
8	WEEK 8

`;
